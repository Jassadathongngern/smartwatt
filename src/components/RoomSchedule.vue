...
const isManager = computed(() => route.path.includes("/admin"));

// --- 1. Date & Time Logistics ---
const today = new Date();
const daysOrder = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
const selectedDay = ref("All"); // Default to All to show full week per room
...
const displayRooms = computed(() => {
  ...
  // Also filter by search so we don't show empty rooms if searching
...
const getGroupedSchedulesForRoom = (room) => {
  ...
  // Get all schedules for this room that match filters
...
<template>
  <div class="schedule-page">
    <div class="header-section">
      <div>
        ...
        <p class="text-gray-500">
          {{ isManager ? "จัดการตารางเรียนตามห้อง" : "ตรวจสอบตารางเรียนและการใช้งาน" }}
        </p>
      </div>
      ...
      <div style="display: flex; gap: 10px">
        ...
        <button v-if="isManager" class="btn-add" @click="openAddModal">+ เพิ่มตารางเรียน</button>
      </div>
    </div>
              ...
              <div class="day-section">
                ...
                <div class="schedule-list">
                  ...
                  <div
                    ...
                    :key="item.id"
                    @click="isManager ? openEditModal(item) : null"
                  >
                    <button
                      v-if="isManager"
    ...
    <RoomManagerModal
      v-if="isManager"
      v-model:showAdd="showAddRoomModal"
      v-model:showList="showRoomListModal"
    />
  </div>
</template>
